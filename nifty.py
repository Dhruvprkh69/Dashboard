...
# Download NSE data first if not already downloaded
...
if st.session_state.nifty_data is None:
    ...
    if all_data:
        st.session_state.nifty_data = pd.DataFrame(all_data)
...
if st.session_state.nifty_data is not None:
    ...
    sort_option = st.radio(
        "Sort by 'Down from High':",
        ["None", "Highest to Lowest", "Lowest to Highest"],
        horizontal=True
    )

    # Sort the DataFrame based on selection
    if sort_option == "Highest to Lowest":
        df = df.sort_values('Down from High', ascending=True)
    elif sort_option == "Lowest to Highest":
        df = df.sort_values('Down from High', ascending=False)
